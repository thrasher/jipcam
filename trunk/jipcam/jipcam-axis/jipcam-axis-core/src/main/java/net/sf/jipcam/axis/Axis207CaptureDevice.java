/*
 * Axis207CaptureDevice.java (2007)
 *
...
/**
 * Describe an Axis207 CaptureDevice.
 * 
 * @author Nigel Simpson
 */
public class Axis207CaptureDevice extends DataSource implements CaptureDevice {
	/**
	 * The full product name as returned from an API query for the Axis
	 * parameter: root.BRAND.PRODFULLNAME
	 */
	public static String PRODUCT_FULL_NAME = "AXIS 207 Network Camera";
	...
	public Axis207CaptureDevice(MediaLocator locator)
			throws MalformedURLException {
		this(null, locator);
	}

	public Axis207CaptureDevice(String deviceName, MediaLocator locator)
			throws MalformedURLException {
		this(deviceName, locator.getURL().getHost(), locator.getURL().getPort());
	}

	/**
	 * Create an Axis 207 device. The device is initially disconnected.
	 * 
	 * @param name
	 * @param hostName
	 * @param port
	 */
	public Axis207CaptureDevice(String deviceName, String hostName, int port) {
		super(
	...
	public CaptureDeviceInfo getCaptureDeviceInfo() {
		// register the device
		// 640x480,480x360,352x288,320x240,240x180,176x144,160x120
		Format[] formats = new Format[7];
		formats[0] = new JPEGFormat(new Dimension(640, 480),
				Format.NOT_SPECIFIED, Format.byteArray, (float) 10,
				Format.NOT_SPECIFIED, Format.NOT_SPECIFIED);
		formats[1] = new JPEGFormat(new Dimension(480, 360),
				Format.NOT_SPECIFIED, Format.byteArray, (float) 10,
				Format.NOT_SPECIFIED, Format.NOT_SPECIFIED);
		formats[2] = new JPEGFormat(new Dimension(352, 288),
				Format.NOT_SPECIFIED, Format.byteArray, (float) 10,
				Format.NOT_SPECIFIED, Format.NOT_SPECIFIED);
		formats[3] = new JPEGFormat(new Dimension(320, 240),
				Format.NOT_SPECIFIED, Format.byteArray, (float) 10,
				Format.NOT_SPECIFIED, Format.NOT_SPECIFIED);
		formats[4] = new JPEGFormat(new Dimension(240, 180),
				Format.NOT_SPECIFIED, Format.byteArray, (float) 10,
				Format.NOT_SPECIFIED, Format.NOT_SPECIFIED);
		formats[5] = new JPEGFormat(new Dimension(176, 144),
				Format.NOT_SPECIFIED, Format.byteArray, (float) 10,
				Format.NOT_SPECIFIED, Format.NOT_SPECIFIED);
		formats[6] = new JPEGFormat(new Dimension(160, 120),
				Format.NOT_SPECIFIED, Format.byteArray, (float) 10,
				Format.NOT_SPECIFIED, Format.NOT_SPECIFIED);
	...
	public static void main(String[] args) {
		try {
			if (args.length != 2) {
				System.out.println("Usage: Axis207CaptureDevice HOST PORT");
				System.exit(1);
			}
			...
			Axis207CaptureDevice cam = new Axis207CaptureDevice(null, host,
					port);
